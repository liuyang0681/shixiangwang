---
title: "TCGA Workflow：使用Bioconductor包分析癌症基因组与表观组学数据"
author: 王诗翔
date: 2018-12-13
slug: "tcga_workflow"
categories: 
...
学习自TCGAWorkflow包英文手册：<https://www.bioconductor.org/packages/release/workflows/vignettes/TCGAWorkflow/inst/doc/TCGAWorkflow.html>，根据我自己的见解和实际操作时的编译情况可能略有增改。
...
该工作流基于文章 [TCGA Workflow: Analyze cancer genomics and epigenomics data using Bioconductor packages](https://f1000research.com/articles/5-1542/v2) [@10.12688/f1000research.8923.2]。由于时间和空间所限，我们仅下载数据的子集，如果想要实战，请使用所有可获取的数据。例子中的数据都可以通过[__TCGAWorkflowData__](https://github.com/BioinformaticsFMRP/TCGAWorkflowData)包获取。


## 安装


如果想要执行工作流的所有步骤，请使用下面代码进行安装：
...
在每一个部分的开头，都需要使用代码加载所需的包。不过，下面的代码是用来加载用于所有部分的包。

这包括：

- TCGAWorkflowData：这个包包含了执行每个分析流程所需的数据，它不是所有的数据，而是一个子集，这样代码的运行才可能快，实际操作请下载所有相应的数据。
- DT：用于可视化结果
- TCGAbiolinks：下载和分析TCGA官方数据
...
测序技术的发展让存储在国际数据库中的公开可获取数据有了爆发式的增长，像[The
Cancer Genome Atlas (TCGA)](http://cancergenome.nih.gov/)、[The
Encyclopedia of DNA Elements (ENCODE)](http://www.encodeproject.org/)、[The NIH Roadmap Epigenomics Mapping Consortium (Roadmap)](http://www.roadmapepigenomics.org/)。这些项目为探索癌症细胞系和正常、肿瘤组织高分辨率的表观基因组提供了前所未有的机会。[Bioconductor](http://www.bioconductor.org/)项目提供了超过1000个开源软件和统计软件包用来分析高通量组学数据。然而，大部分的软件包都设计用来处理特定的数据类型（如表达、表观基因组、基因组），并不存在一个整合3大公开项目资源和数据的综合分析工具。近期，已经有人提出这样的需求。在这个工作流中，我们提供了一系列聚焦于生物学的不同分子数据的整合分析。我们描述了如何下载、处理和准备TCGA数据，操作一些关键的Bioconductor软件包，我们描述如何提取有生物学意义的基因组和表观组数据。使用Roadmap和ENCODE数据，我们提供了一个鉴定与癌症相关联的的生物学相关功能表观元件的工作计划。


为了阐明我们的工作流程，我们分析两种脑癌：low-grade glioma (LGG) 和 high-grade glioma (glioblastoma
multiform or GBM)。

该工作流使用的所有包都可以在[biocViews interface](http://www.bioconductor.org/packages/release/BiocViews.html#___Software)上找到。

**Keywords:** Epigenomics, Genomics, Cancer, non-coding, TCGA, ENCODE, Roadmap, Bioinformatics.

# 介绍

癌症是一种复杂的基因组疾病，它包含多个分子时间，如点突变、结构变异、转位和表观转录特征及网络的激活。这些事件的影响发生在不同的时空尺度上，并伴有相互的交流和反馈机制，因此构成了一个高度复杂的动态系统。为了洞察肿瘤的生物学机制，大多数癌症基因组研究的目的是整合多个不同分子尺度上的观察并进行分析。尽管许多肿瘤有着一些相似的重复基因组事件，它们与观察到的表型却常常难以理解。例如，尽管我们知道大部分的入侵性脑瘤，如glioma包含单个基因（IDH）的突变，它的表观和转录组特征激活机制仍然需要更好地表征和阐述。而且，最近基于网络的策略成为了癌症表型主要调控元件（驱动事件）发现的有效手段。这里我们描述了一个综合的分析流程，它整合了许多Bioconductor软件包去分析和整合位于大样本癌症数据集分子观察的多样性。

的确，最近的技术发展允许我们将大量的基因组和表观组数据（如基因表达、DNA甲基化）存储进公开的国际数据库，如The Cancer Genome
Atlas ([TCGA](http://cancergenome.nih.gov/))、The Encyclopedia of DNA
Elements ([ENCODE](http://www.encodeproject.org/))和NIH Roadmap
Epigenomics Mapping Consortium
([Roadmap](http://www.roadmapepigenomics.org/)) [@Hawkins]。
...
尽管存在很多种访问癌症相关数据的方式[@kannan2015public]，[Bioconductor](http://www.bioconductor.org/)是极好的开源、及时更新、专业用于大规模基因组学数据统计分析的综合性工具集。因此，我们使用Bioconductor来推出我们的工作流程，描述如何下载、处理、分析和整合癌症数据，以此理解癌症相关的特定问题。但是，并不存在单一的工具能够解决通过基因调控网络中序列、突变、表观、表达的整合识别驱动事件识别、癌症发展变异通路。因此，我们的工作流处理基因组和表观数据是建立在多个Bioconductor工具之上的。


# 方法
## 获取数据

TCGA数据可以通过NCI Genomic Data Commons (GDC) [data
portal](https://gdc-portal.nci.nih.gov/)、[GDC Legacy
Archive](https://gdc-portal.nci.nih.gov/legacy-archive/search/f)和
[Broad Institute’s GDAC Firehose](gdac.broadinstitute.org)等数据库获取。

> The GDC Data Portal provides access to the subset of TCGA data that has been
...
access). __Level 1 indicated raw and controlled data, level 2 indicated
...
1 to 3 data, Firehose only provides level 3 and 4__. An explanation of the
...
[GDC data portal](https://gdc-portal.nci.nih.gov/)和[GDC Legacy Archive](https://gdc-portal.nci.nih.gov/legacy-archive/search/f)提供的数据可以使用Bioconductor包[TCGAbiolinks](http://bioconductor.org/packages/TCGAbiolinks/)，而Firehose提供的数据可以通过Bioconductor包
[RTCGAToolbox](http://bioconductor.org/packages/RTCGAToolbox/)获取。

下一步将描述读者如何使用[TCGAbiolinks](http://bioconductor.org/packages/TCGAbiolinks/) &
[RTCGAToolbox](http://bioconductor.org/packages/RTCGAToolbox/)去下载临床信息、基因组数据、转录组数据、表观组数据以及亚型信息和GISTIC结果。这个工作流中使用的所有数据都基于参考基因组版本hg19。

### 从TCGA数据中心下载数据

[TCGAbiolinks](http://bioconductor.org/packages/TCGAbiolinks/)[@TCGAbiolinks]包有3个主要的函数：*GDCquery*、*GDCdownload*和*GDCprepare*，它们依次用于数据的搜索、下载以及载入R。 

*GDCquery*使用[GDC API](https://gdc.cancer.gov/developers/gdc-application-programming-interface-api)去对一个给定的项目和数据分类搜索数据，并可以根据样本、样本类型、文件类型和其他特征进行结果的过滤。该函数返回一个搜索结果和查询参数的汇总表格（包括样本、文件和其他有用信息）。*GDCquery*最重要的参数有：*project*，它接受一个GDC项目名(TCGA-USC, TCGA-LGG, TARGET-AML等)；*data.category*，它接受一个数据分类（Transcriptome Profiling, Copy Number Variation, DNA
methylation, Gene expression等）；*data.type* ，它接受一个数据类型（Gene expression quantification, Isoform Expression Quantification,
miRNA Expression Quantification, Copy Number Segment, Masked Copy Number
Segment等）； *workflow.type*，它接受一个GDC工作流类型（HTSeq - Counts, HTSeq - FPKM-UQ, HTSeq - FPKM等）；*legacy*，它选择legacy数据库（hg19）； *file.type*，它接受一个搜索legacy数据库的文件类型（hg18.seg, hg19.seg, nocnv\_,hg18.seg, nocnv\_hg19.seg,
rsem.genes.results, rsem.genes.normalized\_results等）；*platform*，它接受一个搜索legacy数据库的平台信息（HumanMethylation27, Genome\_Wide\_SNP\_6, IlluminaHiSeq\_RNASeqV2等）。

__完整的参数列表可以在[TCGAbiolinks vignette](https://www.bioconductor.org/packages/3.3/bioc/vignettes/TCGAbiolinks/inst/doc/tcgaBiolinks.html#harmonized-data-1)找到__。

代码列表1展示了该函数的一个例子。

在完成搜索步骤后，用户可以用*GDCdownload*函数下载数据，它支持GDC API或者[gdc client
tools](https://gdc.cancer.gov/access-data/gdc-data-transfer-tool)下载。下载的数据将会保存在一个目录中，该目录名为项目名，子目录是数据类型，如"TCGA-GBM/DNA\_methylation"。

最后，*GDCprepare*函数将下载的数据转换为一个[summarizedExperiment](http://bioconductor.org/packages/SummarizedExperiment/)对象[@huber2015orchestrating]或是数据框。如果*SummarizedExperiment*设置为TRUE，TCGAbiolinks会为对象添加子类型信息和临床信息，它是The Cancer Genome Atlas (TCGA) Research Network reports所定义的（完整列表见 [TCGAquery\_subtype section](http://bioconductor.org/packages/devel/bioc/vignettes/TCGAbiolinks/inst/doc/tcgaBiolinks.html#tcgaquery_subtype-working-with-molecular-subtypes-data)）。代码列表1显示了如何使用这些函数下载hg19版本DNA甲基化和基因表达数据，代码列表2显示下载拷贝数变异数据。其他例子参见[TCGAbiolinks vignette](https://www.bioconductor.org/packages/3.3/bioc/vignettes/TCGAbiolinks/inst/doc/tcgaBiolinks.html)。
...
如果选择了[SummarizedExperiment object](http://www.nature.com/nmeth/journal/v12/n2/fig_tab/nmeth.3252_F2.html)对象，用户可以通过*assay*访问数据信息、*rowRanges*获取每一行的值范围、*colData*获取样本信息（病人、批次、样本类型等）[@huber2015orchestrating; @SummarizedExperiment]。下面代码列表展示了一个例子。
...
使用TCGAbiolinks下载临床数据有两种方式，第一种是仅下载GDC索引临床信息，它包括诊断（生存状态、多少天死亡的、诊断时年龄、随访了多少天、多少天复发）、治疗、统计（性别、种族等）、曝光（吸烟、体重、身高、饮酒历史）信息。该种临床信息可以使用下面代码中的*GDCquery\_clinical*函数获取。这个函数有两个参数：*project* ("TCGA-GBM","TARGET-AML"等)和*type* ("Clinical"
或"Biospecimen"）。第二个方法是下载所有可获取临床信息（就是比前面那种方法多）的XML文件。

>This will give access to all clinical data available which
...
突变信息存储在两种类型的MAF文件中：Protected和Somatic (or Public) MAF文件，它们都来自于GDC注释的VCF文件夹。注释的VCF文件通常有报告突变位点在多个转录本，而protected MAF文件仅报告最有临床意义的突变，somatic MAF过滤低质量突变和germline突变。想要下载Somatic MAF文件，使用[TCGAbiolinks](http://bioconductor.org/packages/TCGAbiolinks/)包的
*GDCquery\_maf*函数。
...
UCEC [@cancer2014comprehensive_gastric])。这些分类可以使用函数*TCGAquery\_subtype*获取数据或者使用*GDCprepare*函数，它会自动将数据整合进项目。
...
Bioconductor包[RTCGAToolbox](http://bioconductor.org/packages/RTCGAToolbox/)
[@samur2014rtcgatoolbox]可以使用函数*getFirehoseData*获取Firehose Level 3和4的数据。下面的参数允许用户选择感兴趣的肿瘤：

-   dataset - 要下载的肿瘤，完整列表使用函数*getFirehoseDatasets*获取

-   runDate - Stddata运行日期，列表使用函数*getFirehoseRunningDates*获取

-   gistic2\_Date - 分析运行日期，使用*getFirehoseAnalyzeDates*函数获取
...
默认，[RTCGAToolbox](http://bioconductor.org/packages/RTCGAToolbox/)允许用户下载至多500MB的数据。想要增加下载文件的大小，用户需要指定*fileSizeLimit*参数。下面代码展示了一个例子。函数*getData*允许用户以一个S4Vector对象访问下载的数据 function（见22-24行）。
...
最后[RTCGAToolbox](http://bioconductor.org/packages/RTCGAToolbox/)可以访问level 4数据，当用户需要GISTIC结果时需要手动指定。GISTIC is used to identify genes targeted by somatic
...
```{r echo=TRUE, message=FALSE, warning=FALSE,fig.height=10,fig.width=15}
gbm.subtypes <- TCGAquery_subtype(tumor = "gbm")
